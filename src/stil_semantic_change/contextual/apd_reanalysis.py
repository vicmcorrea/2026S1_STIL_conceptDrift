...
def load_bert_artifacts(
    ...
    for layer in summary["layers"]:
        ...
        logger.info("Loaded layer %d embeddings: shape %s", layer, layers[layer].shape)
...
def compute_apd_scores(
    ...
    for layer, matrix in embedding_layers.items():
        ...
        for lemma in lemmas:
            ...
            for left_s, right_s in zip(slice_order[:-1], slice_order[1:], strict=False):
            ...
            score_rows.append(
                {
                    ...
                    "sample_count_total": int(sum(slice_counts[s] for s in present)),
                    "sample_count_min": int(min(slice_counts[s] for s in present)),
                }
            )
...
def compare_apd_vs_prt(
    ...
    merged["apd_rank"] = merged.groupby("layer")["apd_primary_drift"].rank(ascending=False)
    merged["prt_rank"] = merged.groupby("layer")["prt_primary_drift"].rank(ascending=False)
    return merged
...
def compute_rank_agreement(comparison: pd.DataFrame) -> list[dict]:
    ...
    for layer, group in comparison.groupby("layer"):
        rho, pval = stats.spearmanr(group["apd_primary_drift"], group["prt_primary_drift"])
        for k in (5, 10, 15):
...
def run_apd_reanalysis(
    ...
    if output_root.resolve() == bert_root.resolve():
        raise ValueError("output_root must differ from bert_root")
    output_root.mkdir(parents=True, exist_ok=True)
    logger.info("loading bert artifacts from %s", bert_root)
    meta, layers, summary, slice_order = load_bert_artifacts(bert_root)
    logger.info(
    ...
    prt_scores_path = bert_root / "scores.parquet"
