...
class PortuguesePreprocessor:
    ...
    def _extract_tokens(self, doc, meta: dict[str, object]) -> list[dict[str, object]]:
        _, token_rows = self._process_document(doc, meta)
        return token_rows

    def _resolve_lemma(self, token, normalized: str) -> str:
    ...
    def _looks_like_malformed_infinitive(self, lemma: str) -> bool:
        return lemma.endswith(("arer", "erer", "irer")) and lemma not in VALID_ARER_ERER_IRER_LEMMAS
