...
def _plot_corpus_profile(
    ...
    vocab = (
        lemma_slice_stats.groupby("slice_id")["lemma"].nunique().rename("vocab_size").reset_index()
    )
...
def _draw_agreement_scatter(
    ...
    texts = [
        ax.text(row[x_col], row[y_col], row["lemma"], fontsize=6.5) for _, row in subset.iterrows()
    ]
    adjust_text(
        texts,
        ax=ax,
    ...
    if add_legend:
        ...
        ax.legend(
            unique.values(), unique.keys(), loc="lower right", frameon=False, fontsize=6, ncol=2
        )
...
def _plot_representative_trajectories(
    ...
    term_data: list[
        tuple[str, pd.DataFrame, pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray, np.ndarray]
    ] = []
    ...
    def _draw_trajectory_panel(
        ...
        ax.fill_between(
            years, w_mean - w_std, w_mean + w_std, color=METHOD_COLORS["Word2Vec"], alpha=0.18
        )
        ax.plot(
    ...
    for idx, (term, t_frame, b_frame, years, w_mean, w_std) in enumerate(term_data):
        ...
        _draw_trajectory_panel(
            pax, term, t_frame, b_frame, years, w_mean, w_std, add_legend=(idx == 0)
        )
        letter = ascii_uppercase[idx].lower()
        _save_bundle(pfig, output_stem.parent / f"{output_stem.name}_panel_{letter}")
